...
function stopCleanupScheduler() {
  if (cleanupIntervalId) {
    ...
    console.log('已停止MCP实例生命周期管理器');
  }
}

// 清理空闲实例
async function cleanupIdleInstances(config) {
  console.log('执行MCP实例空闲清理检查...');
  ...
  try {
    ...
    if (idleInstances.length === 0) {
      console.log('没有发现空闲的MCP实例');
      return;
    }

    console.log(`发现 ${idleInstances.length} 个空闲MCP实例需要清理`);

    // 依次终止每个空闲实例
    for (const instance of idleInstances) {
      try {
        console.log(`正在终止空闲MCP实例 [${instance.instanceId}] ${instance.mcpSession.name}`);
        ...
        console.log(`已成功清理空闲MCP实例 [${instance.instanceId}]`);
      } catch (err) {
        console.error(`清理MCP实例 [${instance.instanceId}] 失败:`, err);
      }
    }
  } catch (err) {
    console.error('MCP实例清理过程中发生错误:', err);
  }
}
